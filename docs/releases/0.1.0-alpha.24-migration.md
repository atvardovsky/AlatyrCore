...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.24`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.23` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.24`
...
To framework version: `0.1.0-alpha.24`
From adapter schema version: `2`
To adapter schema version: `23`
...
To contract SHA-256: `246ef46da2015205adfae1b1f1f9c780d9915fe9a161428fad010b7944e4a7fa`
...
- Review affected target adapters before applying changes and bind update
  evidence to the checked-out target branch and revision.
- Update the installed framework baseline, adapter schema to `23`, target
  template version to `24`, and `debug_mode.contract_version` to `3` when the
  optional module is enabled.
- Install the schema-version-3 Debug authoring template and schema-version-4
  compact index projection for new records.
- Preserve schema-version-1 and version-2 records as migration-limited
  historical evidence. Do not invent materiality, claim fidelity, continuation,
  event roles, or canonical-preservation facts for them.
- Rebuild compact Debug index projections without rewriting existing records.
  Review version-2 lifecycle warnings and keep active records on their original
  schema until they close or are explicitly migrated from evidence.
- Require approval before overwriting existing AI instructions or changing
  protected adapter behavior.
- Run target adapter validation and target-owned checks, or record unresolved
  checks and block acceptance.
...
- Review completed Debug evidence for post-completion events, mismatched trigger
  labels, permissive skip claims, or overstated reproducer fidelity. Preserve
  the historical record and record a repair or superseding continuation rather
  than silently rewriting evidence.
...
Source validation: `python3 tools/check_all.py`; `python3 tools/check_debug_mode.py`; `python3 tools/check_engineering_evidence.py`; `python3 tools/check_target_adapter_validator.py`; `python3 tools/check_release_drift.py --mode change --from-ref HEAD`; `python3 tools/check_release_drift.py --mode release`; `git diff --check`
...
- Structural validation cannot prove event completeness, canonical knowledge
  correctness, materiality judgment, or that an exact external configuration
  was reproduced.
- Schema-version-1 and version-2 records remain less comparable because their
  missing historical facts must not be inferred.
