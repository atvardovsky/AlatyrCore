...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.20`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.19` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.20`
...
To framework version: `0.1.0-alpha.20`
From adapter schema version: `2`
To adapter schema version: `19`
From template version: `3`
To template version: `20`
From contract SHA-256: `fcba36c117b61c893d3ea1ae98a6a5a160672b13e633df9386e8278c5168552d`
To contract SHA-256: `a88009ec2cb788c9450ab5cbec8126a9257e02395f01723ad443ceb4db92514e`
...
- Treat `--allow-placeholders` as a deprecated alias for non-accepting
  `migration-staging`; do not reuse an old zero exit code as accepted evidence.
- Resolve placeholders in required and enabled-capability live surfaces while
  preserving placeholders in explicit reusable authoring templates.
- Add the missing module-profile block or correct its state for every
  manifest-enabled module, and remove manifest/profile false enablement.
- Migrate Debug Mode and durable engineering-evidence indexes to schema 2,
  add `redaction_policy`, and synchronize owner/storage/visibility/retention/
  redaction/external-patch values with their human policy README.
- Bind the migration note, upgrade impact, validator report, and final evidence
  to the checked-out target branch and revision. Validate other branches
  separately when they carry independent adapter state.
- Finish with strict `--validation-phase acceptance`; a staging report cannot
  establish a complete update.
...
- Update `.ai/alatyr.yaml` to adapter schema `19` and template version `20`
  while preserving target-owned facts and records.
- Migrate `.ai/project/engineering-evidence/index.json` and, when enabled,
  `.ai/project/debug/index.json` to schema 2 with explicit redaction policy.
- Resolve live placeholders and machine/human policy projection drift on the
  checked-out target branch.
- Synchronize manifest-enabled modules with their human module-profile blocks.
- Run strict acceptance validation and record the exact branch and revision;
  staging validation is insufficient for completion.
...
Target validation: `migration staging may inventory gaps; strict acceptance is required on each accepted branch/revision`
...
- A successful validation on one target branch does not prove that another
  branch contains the same adapter update.
- Structural checks do not prove target business facts or semantic correctness.
