...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.28`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.27` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.28`
...
To framework version: `0.1.0-alpha.28`
From adapter schema version: `2`
To adapter schema version: `26`
From template version: `3`
To template version: `26`
From contract SHA-256: `fcba36c117b61c893d3ea1ae98a6a5a160672b13e633df9386e8278c5168552d`
To contract SHA-256: `368a698c2d689aa74a6a3440d9f7cdf8acf9c49e9c0839e56a9bd0dc2ddc4d47`
...
- Added schema contracts: 10
- Changed schema contracts: 0
- Removed schema contracts: 0
- Added target template surfaces: 166
...
- Add `installation.state_record` and a machine-readable transition record.
  New installations start at `scaffolded`; older adapters with no record start
  update work at truthful `staged` `legacy-migration-baseline` without
  inventing prior history. Require passed strict validation before `accepted`.
- Preserve existing target-owned shared capability surfaces and apply their
  declared adapter-aware merge strategy. Generic scaffold overwrite mode must
  not replace them.
- Recheck context-cost claims against normalized planned, resolved, and
  observed receipts. Do not treat static word estimates as provider telemetry.
- Recheck assistant conformance evidence against the provider-neutral executor
  lifecycle and record post-install/update delivery as `sent`, `skipped`, or
  `blocked` instead of inferring delivery from template presence.
...
- Rebuild the compact bootstrap and framework inventory after migrating the
  manifest, installation-state record, shared capability surfaces, and output
  contracts.
- Run migration staging while target facts are unresolved, then run strict
  acceptance validation on the exact target branch and revision before moving
  the installation state to `accepted`.
...
Source validation: `python3 tools/check_all.py --profile full` passed 70 source
checks including 109 unit tests; focused release and change drift checks passed.
Target validation: deterministic core, standard, full, and core plus
AI-infrastructure scaffold checks passed; synthetic accepted installation,
blocking drift, staged repair, and post-update acceptance cycles passed. Each
real target still decides and records its own validation.
...
- Provider-neutral executor and routing contracts do not prove unexecuted
  assistant clients follow them. Fresh reviewed runtime evidence remains
  required for provider-behavior claims.
- Static context receipts and cost baselines do not prove real token or
  monetary savings; exact claims require observed host or provider telemetry.
