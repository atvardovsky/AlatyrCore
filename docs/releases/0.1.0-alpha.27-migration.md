...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.27`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.26` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.27`
...
To framework version: `0.1.0-alpha.27`
From adapter schema version: `2`
To adapter schema version: `25`
From template version: `3`
To template version: `25`
From contract SHA-256: `fcba36c117b61c893d3ea1ae98a6a5a160672b13e633df9386e8278c5168552d`
To contract SHA-256: `afa14f91ac00b597af86380ba72e1f970d7473e69e203ebe5f243859834a663d`
...
- Added schema contracts: 8
- Changed schema contracts: 0
- Removed schema contracts: 0
- Added target template surfaces: 165
...
- Review affected target adapters before applying changes and create a target
  migration note.
- Install `ALATYR-KNOWLEDGE-001`, the project-knowledge schemas and target
  policy/index/promotions/routes, routing descriptor, flow, gate, templates,
  operation/help entries, and validator coverage as required core surfaces.
- Advance installed manifest values to adapter schema `25`, template version
  `25`, and context router schema `7` only after migration succeeds.
- Preserve existing engineering evidence, accepted canonical project sources,
  reviewed promotion dispositions, route IDs, contradiction/supersession
  lineage, retention policy, and local deviations. Never replace target state
  with source placeholders or auto-promote historical evidence.
- Resolve target project-knowledge owner, decision authority, source-of-truth
  registry bindings, retention/redaction, packet limits, and freshness policy.
- Regenerate derived bootstrap, operation, framework-pack, and routing surfaces;
  run strict target adapter validation and target project checks.
- Require current-scope authorization before changing existing AI instructions
  or protected adapter behavior.
...
- Review bounded historical engineering evidence for promotion candidates. Keep
  every candidate historical unless target decision authority explicitly
  accepts or narrows it and the canonical owner is updated.
- Run a paired later-task reuse benchmark before claiming context, time, or
  rediscovery savings.
...
- Source selector/conformance checks do not prove semantic truth, human
  acceptance, equivalent assistant strategies, or measured cost savings.
- Hosted CI and exact `v0.1.0-alpha.27` publication can be confirmed only after
