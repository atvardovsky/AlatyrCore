...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.31`.
...
- Update new Debug Mode authoring to schema version 5 and the Debug index to
  schema version 5. Preserve readable schema-version-1 through
  schema-version-4 records as historical evidence; do not rewrite them to
  imply lifecycle coverage or project-knowledge candidate handling that was
  not recorded at the time. Project those older records in the version-5 index
  with `legacy` lifecycle scope, empty covered phases and candidate IDs, and no
  expected continuation.
- For schema-version-5 Debug sessions, record whether the session is active,
  phase-complete, or full-task-complete. When work continues in another
  session, bind the continuation through the Debug index and keep the same
  task lineage while assigning a new authorization scope.
- Give every schema-version-5 project-knowledge candidate an explicit
  disposition. Link promotion proposals to the project-knowledge index and
  preserved discoveries to durable engineering evidence; never infer reuse
  merely from an enabled module or a non-empty index.
- Update engineering-evidence authoring to record schema version 3, use index
  schema version 4, and include reciprocal Debug session IDs when Debug
  evidence contributed to the result.
- Update the project-knowledge index to schema version 3 and set its adoption
  state to `enabled-empty`, `populated`, or `reuse-observed` from actual target
  evidence. An empty index is a valid enabled state and must not be reported as
  demonstrated reuse.
- Update the target manifest contract versions to Debug Mode 5, engineering
  evidence 3, and project knowledge 3 before declaring the adapter current.
