...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.26`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.25` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.26`
...
To framework version: `0.1.0-alpha.26`
...
To contract SHA-256: `de6a464a8ebcd2a47f7132013bf41155556e52835e5643e46af8cddb64a62d29`
...
- Treat evidence from the same framework version as historical when its
  evidence-contract digest differs from the current source contract.
...
Source validation:
`69-check full profile and 70-check release profile passed; 48 unit tests passed
on the host and in clean Python 3.10 and 3.13 containers; 16-check platform
contract passed on the host`
...
- Current-contract real runs do not yet cover every declared assistant surface,
  and the effectiveness task-class suite is not broadly executed.
- Hosted CI and exact `v0.1.0-alpha.26` publication can be confirmed only after
  separately authorized commit, push, workflow, and release-tag actions.
