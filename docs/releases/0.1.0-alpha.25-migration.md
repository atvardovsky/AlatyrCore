...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.25`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.24` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.25`
...
To contract SHA-256: `07f94f27c9b1ad6cd90a63c7d4382525751d9a8132d77a00833d74b25399fcb2`
...
- Review affected target adapters before applying changes and preserve all
  target-owned facts and local deviations.
- Update the installed validator before acceptance so target-relative traversal
  and symlink escapes are rejected consistently.
- Revalidate legacy Debug records. Schema versions 1 and 2 must not contain the
  version-3 `continuation` field; do not invent migration lineage.
- Create or update the target migration note and run
  `recheck-after-framework-update` on the actual target branch.
- Require current-scope approval before overwriting existing AI instructions or
...
- Use the generated source evidence status when making cross-assistant or cost
  claims; historical runs remain historical evidence.
...
Source validation: `full, platform, release-candidate, focused mutation, Python 3.10, and Python 3.13 checks required before release`
...
- Full lifecycle conformance exercises all support profiles with optional
  modules deferred; project-specific enabled-module facts still require their
  focused checks and real target evidence.
- Current-version real runs do not yet cover every declared assistant surface,
  and the effectiveness task-class suite is not broadly executed.
- Hosted CI and exact `v0.1.0-alpha.25` publication can be confirmed only after
  a separately authorized commit, push, and release tag.
