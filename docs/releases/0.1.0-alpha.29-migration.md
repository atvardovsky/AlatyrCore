...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.29`.
Migration reports for intervening releases remain part of the evidence chain.
Inventory sections were produced with `tools/report_migration_diff.py`. This
report is evidence, not authorization to modify, commit, publish, or operate a
target repository.

Report ID: `alatyr-0.1.0-alpha.29`
...
To framework version: `0.1.0-alpha.29`
From adapter schema version: `2`
To adapter schema version: `27`
From template version: `3`
To template version: `27`
From contract SHA-256: `fcba36c117b61c893d3ea1ae98a6a5a160672b13e633df9386e8278c5168552d`
To contract SHA-256: `4e1424879b06a36297e671c8afbc2dd37e7cd12068611f68bfb8a5a117ca1428`
...
- Added target template surfaces: 174
- Changed target template surfaces: 49
...
From framework directory: `v0.1.0-alpha.3:framework/rule-registry.json:framework`
To framework directory: `framework/rule-registry.json:framework`
...
From template directory: `v0.1.0-alpha.3:framework/rule-registry.json:templates/target`
To template directory: `framework/rule-registry.json:templates/target`
...
- Migrate every installed assistant capability record to schema 2 while
  preserving target evidence by surface ID. Add newly registered surfaces as
  unknown rather than copying another client's claims.
- For every selected assistant, inspect native and compatibility instruction
  paths, bind the exact runtime variant, and record selected entry path,
  competing sources, configuration/toggle state, observed auto-load, skills,
  client permissions, diagrams, delegation, and freshness evidence.
- Add or merge `.rules` only when Zed Agent is selected. Preserve existing
  higher-priority target instructions and record any unresolved conflict.
- Do not copy every vendor bridge from a `full` source scaffold. Native bridges
  are now omitted by default and require one `--assistant-surface` selection
  per reviewed target client; preserve existing target-owned instruction
  files and verify actual loading after adaptation.
- Treat Roo Code and Windsurf surfaces as legacy. Do not claim current runtime
  support without exact-client captured evidence.
- Run the target validator on each branch and revision whose adapter state is
  being accepted; branch evidence cannot establish another branch.
