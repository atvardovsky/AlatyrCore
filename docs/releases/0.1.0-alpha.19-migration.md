...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.19`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.18` remain part of the evidence chain. The inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.19`
...
To framework version: `0.1.0-alpha.19`
From adapter schema version: `2`
To adapter schema version: `18`
From template version: `3`
To template version: `19`
From contract SHA-256: `fcba36c117b61c893d3ea1ae98a6a5a160672b13e633df9386e8278c5168552d`
To contract SHA-256: `915b51495adc8b413d609341f4fff6680235bc340e9eb5f4d238b555665e49fd`
...
- Framework version changed because optional Debug Mode now has a portable
  rule owner and explicit activation, authority, attribution, privacy, timing,
  comparison, and finalization semantics.
- Adapter schema version changed because enabled installations declare the
  debug policy, compact index, record store, record schema, flow, gate,
  template, summary, and task-scale overlay paths.
- Target template version changed because the scaffold now includes those
  target-owned surfaces, operation routing, final evidence, update
  preservation, and portable structural validation.
- Debug Mode remains optional and lazy. Existing tasks do not load or write
  debug evidence until a user explicitly enables it for the current task or
  session.
- Activation grants only target-approved non-canonical debug evidence writes;
  it does not grant code, commit, publish, live, protected-change, or tool
  authority.
...
- Install `.ai/framework/debug-mode.md` from the selected framework pack and
  project `ALATYR-DEBUG-001` into the installed rule registry.
- If the target enables Debug Mode, choose its evidence owner, storage,
  visibility, retention, redaction, and external-patch policy before creating
  records.
- Add the target debug index and record store, record/summary templates, lazy
  task-scale overlay, operation, flow, gate, manifest paths, and final-evidence
  fields as one adapter change.
- Keep existing installations inactive by default. Do not infer activation
  from the module state, old records, or a previous task.
- Preserve existing valid records during update, validate index/record and
  causal metric synchronization, and keep Debug Mode artifacts out of clean
  external patches unless target policy explicitly permits them.
