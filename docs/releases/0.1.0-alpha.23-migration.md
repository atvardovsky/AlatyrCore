...
`v0.1.0-alpha.3` with the source tree prepared for `0.1.0-alpha.23`.
Migration reports for intervening `0.1.0-alpha.4` through
`0.1.0-alpha.22` remain part of the evidence chain. Inventory and digest
...
Report ID: `alatyr-0.1.0-alpha.23`
...
To contract SHA-256: `f2066c46573960a15394c6feb22f32ddb1f5463106ea114f0737a40da86410a0`
...
- Set `engineering_evidence.contract_version` and, when enabled,
  `debug_mode.contract_version` to `2`. Install the version-2 authoring
  templates and schema-3 compact indexes as one adapter migration.
- Preserve schema-version-1 Debug and Engineering Evidence records unchanged.
  Do not infer actor, causality, intervention, contribution, binding state, or
  prior-binding lineage for historical records without reviewed evidence.
- Use version 2 for new Debug events. Keep the initial task request in
  activation metadata; classify only a specific direction, expansion,
  constraint, correction, or validation request as an intervention.
- Rebuild Debug index entries with `record_schema_version`,
  `repository_binding_state`, and `engineering_evidence_status`. Rebuild
  Engineering Evidence entries with `record_schema_version` and
  `repository_binding_state` from the selected records.
- Close every completed version-2 Debug Engineering Evidence decision. Capture
  or block material rejected hypotheses and direction-changing corrections, or
  name the canonical project knowledge that justifies skipping capture.
- Recheck final repository bindings with typed Git object resolution, exact
  immutable IDs, commit ancestry, and `prior_bindings` lineage. Treat later
  drift from finalized selected-file snapshots as a warning, not a reason to
  rewrite historical evidence.
- Recheck complete change-package Git ranges for ancestry and preserve their
  finalized snapshot evidence when the current worktree has legitimately
  changed.
- Run the upgrade assessment and installed validator on the checked-out target
  branch. Review validator-affected paths added to `upgrade-impact.json`, apply
  authorized repairs, and rerun strict acceptance on the same branch/revision.
...
- Rebind a selected-file snapshot to a matching immutable commit only through
  an explicit update that appends the prior binding.
...
- Schema-version-1 attribution remains less precise than version 2 and must be
  qualified in cross-task comparisons.
- A historical snapshot warning means current reproducibility is unavailable;
  it does not prove either corruption or continuing semantic correctness.
- Structural attribution and Git checks cannot prove event completeness,
  invariant correctness, or engineering quality.
