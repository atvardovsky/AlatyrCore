...
Create new records with schema version 3. Mark repository bindings provisional
while work is in progress and final only when the object IDs and ancestry are
reproducible. Preserve replaced bindings in `prior_bindings`. Keep schema
versions 1 and 2 unchanged as legacy evidence unless a human-reviewed migration
has enough evidence to populate the new fields.
