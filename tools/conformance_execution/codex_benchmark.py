...
def update_report(
    ...
    *,
    ...
    if not isinstance(receipt, dict):
        raise ValueError(f"{path} must contain a version-2 context receipt")
    observed = receipt.get("observed")
    if not isinstance(observed, dict):
        raise ValueError(f"{path} context receipt must contain observed evidence")
    ...
    observed.update(
        {
            ...
            "evidence": f"Codex CLI turn.completed usage for thread {thread_id}",
        }
    )
    receipt["measurement_state"] = "observed"
    receipt_failures = validate_context_receipt(receipt, f"{path} context_receipt")
    if receipt_failures:
    ...
    report["context_measurement_kind"] = (
        "provider-observed completion-event tokens with assistant-resolved file context"
    )
    report["input_tokens"] = input_tokens
    report["output_tokens"] = output_tokens
    report["context_files_loaded"] = observed.get("files_loaded", "unknown")
...
def run_benchmark(args: argparse.Namespace) -> int:
    ...
    record = new_execution_record(
        ...
        run_id=str(manifest.get("benchmark_id", "unknown")),
        source_commit=str(manifest.get("source_commit", "unknown")),
        record_kind="effectiveness-benchmark-execution",
    )
    ...
    record_execution(
        ...
        detail=f"Codex CLI executed {len(executions)} isolated benchmark process(es).",
    )
    if failed:
        write_execution_record(record_path, record)
        return 1
    collect_reports(
        record,
        [Path(str(run["report"])) for run in manifest.get("runs", []) if isinstance(run, dict)],
    )
    write_execution_record(record_path, record)
    ...
    record_validation(
        ...
        detail="check_effectiveness_benchmark.py --require-reports completed",
    )
